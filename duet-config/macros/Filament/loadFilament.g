...
M291 S3 P{"Click OK to heat to " ^ {move.axes[state.currentTool].workplaceOffsets[3]} ^ "C and load " ^ move.extruders[state.currentTool].filament} R{"Load " ^ move.extruders[state.currentTool].filament ^ "?"}

; Set current the configured temperature and wait for the extruder to reach temp
M109 S{move.axes[state.currentTool].workplaceOffsets[3]}
...
; G1 E15 F{move.axes[state.currentTool].workplaceOffsets[4]} ; Feed 15mm of filament slowly
G1 E15 F300

; purge and ask for more loop
while true
    ; G1 E30 F{move.axes[state.currentTool].workplaceOffsets[4]} ; Feed 30mm of filament slowly
    G1 E15 F300
